...
contract Multipliers {
    ...
    function pay() private {
        ...
        uint128 money = 0;
        if(balance > prizeAmount) money = uint128(balance - prizeAmount);
    ...
    function addDeposit(address depositor, uint value) private {
        ...
        if(value >= MIN_INVESTMENT_FOR_PRIZE) lastDepositInfo = LastDepositInfo(uint128(currentQueueSize), uint128(now));
        ...
        require(TECH.send(support));
        uint adv = value * PROMO_PERCENT / 100;
        require(PROMO.send(adv));
    }
    ...
    function withdrawPrize() private {
        ...
        uint balance = address(this).balance;
        if(prizeAmount > balance) prizeAmount = balance;
        uint donation = prizeAmount * FATHER_PERCENT /(FATHER_PERCENT + PRIZE_PERCENT);
        if(donation > 10 ether) donation = 10 ether;
        require(gasleft() >= 300000, "We need gas for the father contract");
        require(FATHER.call.value(donation).gas(250000)());
        uint prize = prizeAmount - donation;
        require(queue[lastDepositInfo.index].depositor.send(prize));
        prizeAmount = 0;
    ...
    function push(address depositor, uint deposit, uint expect) private {
        ...
        if(queue.length == currentQueueSize) queue.push(dep);
        else queue[currentQueueSize] = dep;
        currentQueueSize ++;
    }
    ...
    function getDepositorMultiplier(address depositor) public view returns(uint) {
        DepositCount storage c = depositsMade[depositor];
        uint count = 0;
        if(c.stage == getCurrentStageByTime()) count = c.count;
        if(count < MULTIPLIERS.length) return MULTIPLIERS[count];
