...
contract NamiPool {
    ...
    event UpdateShareHolder(address ShareHolderAddress, uint RoundIndex, uint Stake, uint Time);
    event Deposit(address sender, uint RoundIndex, uint value);
    event WithdrawPool(uint Amount, uint TimeWithdraw);
    event UpdateActive(address ShareHolderAddress, uint RoundIndex, bool Status, uint Time);
    event Withdraw(address ShareHolderAddress, uint RoundIndex, uint Ether, uint Nac, uint TimeWithdraw);
    event ActivateRound(uint RoundIndex, uint TimeActive);
    function changeEscrow(address _escrow) onlyNamiMultisig public {
        require(_escrow != 0x0);
        escrow = _escrow;
    }
    function withdrawEther(uint _amount) onlyEscrow public {
        require(namiMultiSigWallet != 0x0);
        if(address(this).balance > 0) {
            namiMultiSigWallet.transfer(_amount);
        }
    }
    function withdrawNAC(uint _amount) onlyEscrow public {
    ...
    function depositEthPool(uint _roundIndex) onlyEscrow public payable {
    ...
    function withdrawEthPool(uint _roundIndex, uint _amount) onlyEscrow public {
    ...
    function closeEthPool(uint _roundIndex) onlyEscrow public {
...
contract NamiCrowdSale {
    ...
    uint constant public TOKEN_SUPPLY_LIMIT = 1000000000 *(1 ether / 1 wei);
    uint public binary = 0;
    enum Phase {
        Created, Running, Paused, Migrating, Migrated }
    ...
    function transferForTeam(address _to, uint256 _value) onlyEscrow public {
        _transfer(msg.sender, _to, _value);
    }
    function transfer(address _to, uint256 _value) onlyTranferable public {
        _transfer(msg.sender, _to, _value);
    }
    function transferFrom(address _from, address _to, uint256 _value) onlyTranferable public returns(bool success) {
        require(_value <= allowance[_from][msg.sender]);
        allowance[_from][msg.sender] -= _value;
        _transfer(_from, _to, _value);
        return true;
    }
    function approve(address _spender, uint256 _value) onlyTranferable public returns(bool success) {
        allowance[msg.sender][_spender] = _value;
        return true;
    }
    function approveAndCall(address _spender, uint256 _value, bytes _extraData) onlyTranferable public returns(bool success) {
    ...
    function changeTransferable() onlyEscrow public {
    ...
    function changeBinary(uint _binary) onlyEscrow public {
        binary = _binary;
    }
    function changeBinaryAddress(address _binaryAddress) onlyEscrow public {
    ...
    function burnTokens(address _owner) onlyCrowdsaleManager public {
    ...
    function setPresalePhase(Phase _nextPhase) onlyEscrow public {
    ...
    function safeWithdraw(address _withdraw, uint _amount) onlyEscrow public {
    ...
    function setCrowdsaleManager(address _mgr) onlyEscrow public {
        require(currentPhase != Phase.Migrating);
    ...
    function migrateToken(address _from, address _to) onlyEscrow public {
        _migrateToken(_from, _to);
    }
    function migrateForInvestor() public {
        _migrateToken(msg.sender, msg.sender);
    }
    event TransferToBuyer(address _from, address _to, uint _value, address _seller);
    event TransferToExchange(address _from, address _to, uint _value, uint _price);
...
contract BinaryOption {
    bool private lock_for_reentrancy;
    ...
    uint constant public MAX_INVESTOR = 20;
    uint public minimunEth = 10000000000000000;
    event SessionOpen(uint timeOpen, uint sessionId);
    event InvestClose(uint timeInvestClose, uint priceOpen, uint sessionId);
    event Invest(address investor, bool choose, uint amount, uint timeInvest, uint sessionId);
    event SessionClose(uint timeClose, uint sessionId, uint priceClose, uint nacPrice, uint rateWin, uint rateLoss, uint rateFee);
    ...
    function changeMinEth(uint _minimunEth) onlyEscrow public {
        require(_minimunEth != 0);
        minimunEth = _minimunEth;
    }
    function changeTimeInvest(uint _timeInvest) onlyEscrow public {
        require(! session.isOpen && _timeInvest < timeOneSession);
        timeInvestInMinute = _timeInvest;
    }
    function changeTimeOneSession(uint _timeOneSession) onlyEscrow public {
        require(! session.isOpen && _timeOneSession > timeInvestInMinute);
        timeOneSession = _timeOneSession;
    }
    function changeRateWin(uint _rateWin) onlyEscrow public {
        require(! session.isOpen);
        rateWin = _rateWin;
    }
    function changeRateLoss(uint _rateLoss) onlyEscrow public {
        require(! session.isOpen);
        rateLoss = _rateLoss;
    }
    function changeRateFee(uint _rateFee) onlyEscrow public {
        require(! session.isOpen);
        rateFee = _rateFee;
    }
    function withdrawEther(uint _amount) onlyEscrow public {
    ...
    function resetSession() onlyEscrow public {
    ...
    function openSession() onlyEscrow public {
        require(session.isReset && ! session.isOpen);
    ...
    function closeInvest(uint _priceOpen) onlyEscrow public {
    ...
    function closeSession(uint _priceClose) onlyEscrow public {
...
contract BinaryOptionV2 {
    ...
    event SessionOpen(uint timeOpen, uint sessionId);
    event InvestClose(uint timeInvestClose, uint priceOpen, uint sessionId);
    event Invest(address investor, uint choose, uint amount, uint timeInvest, uint sessionId);
    event InvestToPool(address investor, uint amount, uint timeInvest);
    event SessionClose(uint timeClose, uint sessionId, uint priceClose, uint rateWin, uint rateLoss, uint rateFee);
    event Deposit(address sender, uint value);
    function () public payable {
        if(msg.value > 0) emit Deposit(msg.sender, msg.value);
    }
    ...
    function changeMinNac(uint _minNAC) onlyEscrow public {
        require(_minNAC != 0);
        minNac = _minNAC;
    }
    function changeTimeInvest(uint _timeInvest) onlyEscrow public {
    ...
    function changeTradableFciInSession(bool _isTradableFciInPool) onlyEscrow public {
    ...
    function getChooses() public view returns(uint[20]) {
    ...
    function setPoolStatus() onlyEscrow public {
        NamiCrowdSale namiToken = NamiCrowdSale(NamiAddr);
        if(namiToken.balanceOf(address(this)) == 0) {
            isEmptyPool = true;
        }
...
contract NamiExchange {
    ...
    event BuyHistory(address buyer, address seller, uint price, uint volume, uint time);
    event SellHistory(address seller, address buyer, uint price, uint volume, uint time);
    ...
    function buyNac(address _seller, uint _price) public payable returns(bool success) {
...
contract NamiMultiSigWallet {
    ...
    function NamiMultiSigWallet(address[] _owners, uint _required) validRequirement(_owners.length, _required) public {
        for(uint i = 0;
        i < _owners.length;
        i ++) {
    ...
    function addOwner(address owner) onlyWallet ownerDoesNotExist(owner) notNull(owner) validRequirement(owners.length + 1, required) public {
        isOwner[owner] = true;
        owners.push(owner);
    ...
    function replaceOwner(address owner, address newOwner) onlyWallet ownerExists(owner) ownerDoesNotExist(newOwner) public {
        for(uint i = 0;
        i < owners.length;
        i ++) {
            if(owners[i] == owner) {
    ...
    function changeRequirement(uint _required) onlyWallet validRequirement(owners.length, _required) public {
        required = _required;
    ...
    function confirmTransaction(uint transactionId) ownerExists(msg.sender) transactionExists(transactionId) notConfirmed(transactionId, msg.sender) public {
        confirmations[transactionId][msg.sender] = true;
        emit Confirmation(msg.sender, transactionId);
        executeTransaction(transactionId);
    }
    function revokeConfirmation(uint transactionId) ownerExists(msg.sender) confirmed(transactionId, msg.sender) notExecuted(transactionId) public {
        confirmations[transactionId][msg.sender] = false;
        emit Revocation(msg.sender, transactionId);
    }
    function executeTransaction(uint transactionId) notExecuted(transactionId) public {
    ...
    function addTransaction(address destination, uint value, bytes data) notNull(destination) internal returns(uint transactionId) {
