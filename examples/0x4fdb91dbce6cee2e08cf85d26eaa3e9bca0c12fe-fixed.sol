...
contract DividendManagerInterface {
    function payDividend() payable external;
}
contract BlackBoxInterface {
    function createGen0(uint _unicornId) payable public;
    function geneCore(uint _childUnicornId, uint _parent1UnicornId, uint _parent2UnicornId) payable public;
}
...
contract UnicornBreeding is UnicornAccessControl {
    ...
    event HybridizationAdd(uint unicornId, uint price);
    event HybridizationAccept(uint firstUnicornId, uint secondUnicornId, uint newUnicornId);
    event HybridizationDelete(uint unicornId);
    event FundsTransferred(address dividendManager, uint value);
    event CreateUnicorn(address owner, uint unicornId, uint parent1, uint parent2);
    event NewGen0Limit(uint limit);
    event NewGen0Step(uint step);
    event OfferAdd(uint256 unicornId, uint price);
    event OfferDelete(uint256 unicornId);
    event UnicornSold(uint256 unicornId);
    ...
    function makeHybridization(uint _unicornId, uint _price) public {
        ...
        hybridizations[_unicornId] = Hybridization({ price : _price, exists : true, listIndex : hybridizationListSize });
    ...
    function createUnicorn() whenNotPaused public payable returns(uint256) {
    ...
    function createPresaleUnicorns(uint _count, address _owner) onlyManager whenPaused public payable returns(bool) {
    ...
    function setGen0Limit() onlyCommunity external {
        require(gen0Count == gen0Limit);
        gen0Limit = gen0Limit.add(gen0Step);
        emit NewGen0Limit(gen0Limit);
    }
    function setGen0Step(uint _step) onlyCommunity external {
        gen0Step = _step;
    ...
    function sellUnicorn(uint _unicornId, uint _price) public {
        ...
        offers[_unicornId] = Offer({ price : _price, exists : true, marketIndex : marketSize });
