...
contract DSNote {
    event LogNote(bytes4 sig, address guy, bytes32 foo, bytes32 bar, uint wad, bytes fax);
    modifier note {
        bytes32 foo;
        bytes32 bar;
        assembly {
...
contract EventfulMarket {
    ...
    event LogTrade(uint pay_amt, address pay_gem, uint buy_amt, address buy_gem);
    event LogMake(bytes32 id, bytes32 pair, address maker, ERC20 pay_gem, ERC20 buy_gem, uint128 pay_amt, uint128 buy_amt, uint64 timestamp);
    event LogBump(bytes32 id, bytes32 pair, address maker, ERC20 pay_gem, ERC20 buy_gem, uint128 pay_amt, uint128 buy_amt, uint64 timestamp);
    event LogTake(bytes32 id, bytes32 pair, address maker, ERC20 pay_gem, ERC20 buy_gem, address taker, uint128 take_amt, uint128 give_amt, uint64 timestamp);
    event LogKill(bytes32 id, bytes32 pair, address maker, ERC20 pay_gem, ERC20 buy_gem, uint128 pay_amt, uint128 buy_amt, uint64 timestamp);
}
contract SimpleMarket is EventfulMarket, DSMath {
    ...
    function bump(bytes32 id_) can_buy(uint256(id_)) {
        var id = uint256(id_);
        LogBump(id_, sha3(offers[id].pay_gem, offers[id].buy_gem), offers[id].owner, offers[id].pay_gem, offers[id].buy_gem, uint128(offers[id].pay_amt), uint128(offers[id].buy_amt), offers[id].timestamp);
    }
    function buy(uint id, uint quantity) can_buy(id) synchronized returns(bool) {
        ...
        LogItemUpdate(id);
        LogTake(bytes32(id), sha3(offer.pay_gem, offer.buy_gem), offer.owner, offer.pay_gem, offer.buy_gem, msg.sender, uint128(quantity), uint128(spend), uint64(now));
    ...
    function cancel(uint id) can_cancel(id) synchronized returns(bool success) {
        OfferInfo memory offer = offers[id];
        delete offers[id];
        assert(offer.pay_gem.transfer(offer.owner, offer.pay_amt));
        LogItemUpdate(id);
        LogKill(bytes32(id), sha3(offer.pay_gem, offer.buy_gem), offer.owner, offer.pay_gem, offer.buy_gem, uint128(offer.pay_amt), uint128(offer.buy_amt), uint64(now));
        success = true;
    }
    function kill(bytes32 id) {
        assert(cancel(uint256(id)));
    }
    function make(ERC20 pay_gem, ERC20 buy_gem, uint128 pay_amt, uint128 buy_amt) returns(bytes32 id) {
    ...
    function offer(uint pay_amt, ERC20 pay_gem, uint buy_amt, ERC20 buy_gem) can_offer synchronized returns(uint id) {
        ...
        LogItemUpdate(id);
        LogMake(bytes32(id), sha3(pay_gem, buy_gem), msg.sender, pay_gem, buy_gem, uint128(pay_amt), uint128(buy_amt), uint64(now));
    }
...
contract MatchingMarket is MatchingEvents, ExpiringMarket, DSNote {
    ...
    function make(ERC20 pay_gem, ERC20 buy_gem, uint128 pay_amt, uint128 buy_amt) returns(bytes32) {
        return bytes32(offer(pay_amt, pay_gem, buy_amt, buy_gem));
    }
    function take(bytes32 id, uint128 maxTakeAmount) {
        assert(buy(uint256(id), maxTakeAmount));
    ...
    function offer(uint pay_amt, ERC20 pay_gem, uint buy_amt, ERC20 buy_gem) isWhitelist(pay_gem, buy_gem) returns(uint) {
        var fn = matchingEnabled ? _offeru : super.offer;
        return fn(pay_amt, pay_gem, buy_amt, buy_gem);
    }
    function offer(uint pay_amt, ERC20 pay_gem, uint buy_amt, ERC20 buy_gem, uint pos) isWhitelist(pay_gem, buy_gem) can_offer returns(uint) {
        return offer(pay_amt, pay_gem, buy_amt, buy_gem, pos, false);
    }
    function offer(uint pay_amt, ERC20 pay_gem, uint buy_amt, ERC20 buy_gem, uint pos, bool rounding) isWhitelist(pay_gem, buy_gem) can_offer returns(uint) {
    ...
    function buy(uint id, uint amount) can_buy(id) returns(bool) {
        var fn = matchingEnabled ? _buys : super.buy;
        return fn(id, amount);
    }
    function cancel(uint id) can_cancel(id) returns(bool success) {
        if(matchingEnabled) {
            if(isOfferSorted(id)) {
                assert(_unsort(id));
            }
    ...
    function insert(uint id, uint pos) returns(bool) {
    ...
    function addTokenPairWhitelist(ERC20 baseToken, ERC20 quoteToken) auth note public returns(bool) {
    ...
    function remTokenPairWhitelist(ERC20 baseToken, ERC20 quoteToken) auth note public returns(bool) {
        require(isTokenPairWhitelisted(baseToken, quoteToken));
    ...
    function setMinSell(ERC20 pay_gem, uint dust) auth note returns(bool) {
        _dust[pay_gem] = dust;
        LogMinSell(pay_gem, dust);
        return true;
    }
    function getMinSell(ERC20 pay_gem) constant returns(uint) {
    ...
    function _find(uint id) internal returns(uint) {
        require(id > 0);
        address buy_gem = address(offers[id].buy_gem);
        address pay_gem = address(offers[id].pay_gem);
    ...
    function _isLtOrEq(uint low, uint high) internal returns(bool) {
        return mul(offers[low].buy_amt, offers[high].pay_amt) >= mul(offers[high].buy_amt, offers[low].pay_amt);
    }
    function _matcho(uint t_pay_amt, ERC20 t_pay_gem, uint t_buy_amt, ERC20 t_buy_gem, uint pos, bool rounding) internal returns(uint id) {
        uint best_maker_id;
        uint t_buy_amt_old;
        uint m_buy_amt;
        uint m_pay_amt;
        require(pos == 0 || ! isActive(pos) || t_buy_gem == offers[pos].buy_gem && t_pay_gem == offers[pos].pay_gem);
        while(_best[t_buy_gem][t_pay_gem] > 0) {
            best_maker_id = _best[t_buy_gem][t_pay_gem];
            m_buy_amt = offers[best_maker_id].buy_amt;
            m_pay_amt = offers[best_maker_id].pay_amt;
            if(mul(m_buy_amt, t_buy_amt) > mul(t_pay_amt, m_pay_amt) +(rounding ? m_buy_amt + t_buy_amt + t_pay_amt + m_pay_amt : 0)) {
                break;
            }
    ...
    function _sort(uint id, uint pos) internal {
        ...
        address pay_gem = address(offers[id].pay_gem);
        uint prev_id;
        if(pos == 0 || ! isActive(pos) || ! _isLtOrEq(id, pos) ||(_rank[pos].prev != 0 && _isLtOrEq(id, _rank[pos].prev))) {
            pos = _find(id);
        }
